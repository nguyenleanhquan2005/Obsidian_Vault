...
# 🎙️ Digital Speech Processing (DSP) Master Hub

> [!ABSTRACT] **Executive Summary** Hệ thống tri thức này tập trung vào việc hiểu và mô phỏng cách con người tạo ra, cảm nhận và xử lý tín hiệu tiếng nói. Từ nền tảng toán học của **DSP**, chúng ta tiến tới việc trích xuất đặc trưng trong **Miền thời gian (Time-domain)** và **Miền tần số (Frequency-domain)** để phục vụ các ứng dụng hiện đại như **ASR (Nhận dạng)** và **TTS (Tổng hợp)**.

---

## 🗺️ Bản đồ lộ trình (Knowledge Roadmap)

## 🟢 Giai đoạn 1: Nền tảng & Cơ sở sinh học (Phần 1)

-  [[Chapter 01_Digital Speech Processing]] | Tổng quan hệ thống
    
-  [[Chapter 02_Review of Fundamentals of Digital Signal Processing]] | Toán học & Bộ lọc
    
-  [[Chapter 03_Fundamentals of Human Speech Production]] | Cơ chế tạo âm
    
-  [[Chapter 04_Hearing, Auditory Models, and Speech Perception]] | Cơ chế thính giác
    
-  [[Chapter 05_Sound Propagation in the Human Vocal Tract]] | Lan truyền sóng âm
    

## 🟡 Giai đoạn 2: Phân tích & Trích xuất đặc trưng

-  [[Chapter 06_Time-Domain Methods for Speech Processing]] | Năng lượng, ZCR, Pitch
    
-  [[Chapter 07_Frequency-Domain Representations]] | STFT & Spectrogram
    
-  [[Chapter 08_The Cepstrum and Homomorphic Speech Processing]] | MFCC & Tách nguồn
    
-  [[Chapter 09_Linear Predictive Analysis of Speech Signals]] | Mô hình LPC
    
-  [[Chapter 10_Algorithms for Estimating Speech Parameters]] | Ước lượng tham số
    

## 🔵 Giai đoạn 3: Ứng dụng & Công nghệ cao

-  [[Chapter 11_Digital Coding of Speech Signals]] | Nén & Truyền dẫn
    
-  [[Chapter 12_Frequency-Domain Coding of Speech and Audio]] | Mã hóa cao cấp
    
-  [[Chapter 13_Text-to-Speech Synthesis Methods]] | Tổng hợp tiếng nói (TTS)
    
-  [[Chapter 14_Automatic Speech Recognition and NLU]] | Nhận dạng & Hiểu ngôn ngữ (ASR/NLU)
    

---

## 🧪 Các trụ cột kỹ thuật cốt lõi (Core Pillars)

> [!QUOTE] **Từ Tín hiệu vật lý đến Dữ liệu số** Quá trình chuyển đổi dựa trên định lý **Nyquist-Shannon** (Fs​≥2Fmax​) và quy trình **Lấy mẫu/Lượng tử hóa**.

## 1. Cơ chế cấu âm (Articulatory System)

Dựa trên sự phối hợp giữa luồng khí phổi và các bộ phận cấu âm:

- **Vowels (Nguyên âm):** Luồng khí tự do, thay đổi bởi sự cộng hưởng (Formants F1, F2).
    
- **Consonants (Phụ âm):** Luồng khí bị cản trở/chặn đứng tại các vị trí cấu âm.
    
- **Voicing:** Sự rung (Voiced) hoặc không rung (Voiceless) của dây thanh đới.
    

## 2. Phân tích Miền thời gian (Time-Domain Analysis)

Các kỹ thuật cơ bản để xử lý nhanh trên khung (Frame):

- **STE (Short-Time Energy):** Phát hiện đoạn có tiếng nói (VAD).
    
- **ZCR (Zero-Crossing Rate):** Phân biệt âm hữu thanh (ZCR thấp) và vô thanh (ZCR cao).
    
- **Autocorrelation:** Thuật toán chủ đạo để ước lượng **Pitch (F0)**.
    

## 3. Phân tích Miền tần số (Frequency-Domain)

- **STFT:** Cắt tín hiệu thành các đoạn ngắn (20-30ms) để bắt được tính không dừng (non-stationary) của tiếng nói.
    
- **Spectrogram:** "Bản đồ" 2D hiển thị cường độ âm thanh theo thời gian và tần số.
    

---

## 🛠️ Công cụ & Thực hành (Tech Stack)

- **Ngôn ngữ:** Python (Librosa, Scipy.signal, Praat).
    
- **Mô hình:** CNN (dựa trên Scalogram/Spectrogram), RNN/Transformer (cho ASR).
    
- **Đánh giá:** SNR (Tỷ lệ tín hiệu trên nhiễu), WER (Tỷ lệ lỗi từ).
    

---

## 🔗 Liên kết nhanh & Ghi chú

- #DSP #SpeechProcessing #AI #AudioEngineering
    
- **Ghi chú quan trọng:** Luôn chú ý sự khác biệt giữa _Spectrogram_ (STFT - Chapter 7) và _Scalogram_ (CWT - nâng cao) khi huấn luyện CNN.
